...
> **Which setup do I need?** See **`/CONFIGURE.md`** (top-level) for the decision tree and the **with/without sourcebooks × with/without published adventure** matrix, and **`configure-matrix.md`** (next to this file) for the step-by-step per-quadrant walkthroughs. The files below are the building blocks those guides call.
